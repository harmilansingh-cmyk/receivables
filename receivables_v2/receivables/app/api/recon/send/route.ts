...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!SCRIPT_ID) return NextResponse.json({ ok:false, manual:true, message:`Run sendRequestEmails("${month}") in Apps Script.` })
    const data = await runScript('sendRequestEmails', [month])
    if (data.error) return NextResponse.json({ ok:false, manual:true, error:data.error.message, message:`Run sendRequestEmails("${month}") in Apps Script.` })
    return NextResponse.json({ ok:true })
  } catch(e) { return NextResponse.json({ ok:false, manual:true, error:String(e), message:'Run sendRequestEmails() in Apps Script.' }) }
}
